...
class cortex_model:
    ...
    def STDP_dynamics(self, t_sim: int) -> None:
        ...
        indices = np.random.randint(low = 0 , high = self.size, size = 2  , dtype = int)
        ...
        # updates topology
        if np.sum(n_s):
            ratio = c_ij / np.sum(n_s)
            if ratio >= self.theta_stdp and (not self.graph.hasEdge(indices[1], indices[0])):
                self.graph.addEdge(indices[1], indices[0])
                self.g_s[indices[1], indices[0]] = self.g_levels
                self.post_syn_neurons[indices[1]] = None
            elif ratio < self.theta_stdp and (self.graph.hasEdge(indices[1], indices[0])):
